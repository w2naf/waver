...
IF (nSteps MOD 2) EQ 0 THEN ++nSteps    ;Force an odd number of times to make FFT work easier.
...
FOR winKK=0,nSteps-1 DO BEGIN
    ...
    RAD_DEFINE_BEAMS,stID,nBeams,nGates,scan_startYear,scan_startYrSec      $
        ,COORDS             = 'rang'                                        $
        ,HEIGHT             = height                                        $
        ,BMSEP              = bmSep                                         $
        ,LAGFR0             = lagFr                                         $
        ,SMSEP0             = smsep                                         $
        ,FOV_LOC_CENTER     = ctrRangeArr
    ctrRangeArr = ctrRangeArr[*,*,0:nGates-1]
    ;ctrRangeArr[2,*,*] - 0 is geographic, 1 is geomagnetic
...
;    IF beamRange[0] GT 0 THEN $
;        interpArr[0:beamRange[0]-1,*]           = !VALUES.F_NAN
;
;    IF beamRange[1] LT nBeams-1 THEN $
;        interpArr[beamRange[1]+1:nBeams-1,*]      = !VALUES.F_NAN
;
;    IF gateRange[0] GT 0 THEN $
;        interpArr[*,0:gateRange[0]-1]           = !VALUES.F_NAN
;
;    IF gateRange[1] LT nGates-1 THEN $
;        interpArr[*,gateRange[1]+1:nGates-1]      = !VALUES.F_NAN
...
;    jp,scan_startJul
;    print,[dataArr[0,*],interpArr[0,*],clrArr[0,*],dataArr[1,*],interpArr[1,*],clrArr[1,*]]
;    PS_CLOSE
